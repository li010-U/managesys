import pathlib
p = pathlib.Path('src/views/environment/EnvironmentView.vue')
p.write_text("""<template>
  <div class=\"env-page\">
    ...
    <el-card shadow=\"never\" class=\"sensor-grid-card\">
      <div v-if=\"loading\" class=\"loading-overlay\"><el-icon class=\"is-loading\" :size=\"32\"><Loading2 /></el-icon></div>
      ...
      <div v-else class=\"sensor-grid\">
        <div v-for=\"s in filteredSensors\" :key=\"s.id\" class=\"sensor-card\" :class=\"sensorCardClass(s)\">
          ...
          <div class=\"sc-body\">
            <div class=\"sc-value\">
              <template v-if=\"s.current_value && s.current_value.value !== null && s.current_value.value !== undefined\">
                <span class=\"sc-value-num\" :style=\"{ color: sensorValueColor(s) }\">{{ formatValue(s.current_value.value, s.sensor_type) }}</span>
                <span class=\"sc-value-unit\">{{ s.current_value.unit }}</span>
              </template>
              <span v-else class=\"sc-value-na\">--</span>
            </div>
            <div class=\"sc-meta\">
              <span v-if=\"s.last_update_time\" class=\"sc-time\">更新: {{ formatTime(s.last_update_time) }}</span>
              <span v-if=\"s.threshold_max\" class=\"sc-threshold\">上限: {{ s.threshold_max }}{{ getUnit(s.sensor_type) }}</span>
            </div>
...
async function submit() {
  ...
  try {
    ...
    if (data.threshold_min === \"\" || data.threshold_min === null) data.threshold_min = undefined
    if (data.threshold_max === \"\" || data.threshold_max === null) data.threshold_max = undefined
